<template>
    <div class="wrapper">
        <el-table class="table" :data="workFinishSList" border>
            ...
            <el-table-column label="操作">
                <template scope="scope">
                    <el-button size="small" type="success" v-on:click="start(scope.$index, scope.row)">查看作业</el-button>
                </template>           
            </el-table-column>
        </el-table>
    </div>
  ...
  created: function() {
    this.$http({
      ...
      if (response.data.result === 0) {
        this.workFinishSList = response.data.data.homeworks;
      } else {
        this.$notify({
          title: "未知错误！",
          type: "success",
    ...
    // this.$http({
    //     method: 'POST',
    //     url: '/api/student/WorkFinishSList',
    //     headers: {
    //         'Authorization': 'Bearer '+ localStorage.token
    //     }
    // }).then( response => {
    //     if(response.data.errno == 200) {
    //         this.workFinishSList = response.data.data
    //     }
    //     else {
    //         this.$notify({
    //             title: '未知错误！',
