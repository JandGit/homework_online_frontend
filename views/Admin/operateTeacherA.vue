<template>
  <div class="wrapper">
  <el-input class="searchInput" placeholder="请输入要搜索的任意关键字" icon="search" v-model="searchTeacher" :focus="watchEnter()"  v-on:click="search()"></el-input>
    <el-table class="table" :data="queryTeachers" border>
        <el-table-column label="教师编号" width="180" prop="id"></el-table-column>
        <el-table-column label="教师姓名" width="180" prop="name"></el-table-column>
  ...
  export default {
    created: function() {
      this.$http({
              ...
              // url: '/api/admin/operateTeacherA',
              headers: {
                  'Authorization': 'Bearer '+ localStorage.token
              }
          }).then( response => {
              if(response.data.errno == 200) {
                  this.teachers = response.data.operateTeacherA,
    ...
    methods: {
      ...
      addTeacher :function(){
             ...
             if(this.teacher.name != '' && this.teacher.class != ''){
                  this.$http({
                  method: 'get',
                  url: '/static/admin.json',
                  // url: '/api/admin/addTeacherA',
                  headers: {
                           'Authorization': 'Bearer '+ localStorage.token
                  },
                  body: {
